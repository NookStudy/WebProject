...
$(function () {	
	<c:if test="${empty flag}" var="result">
		alert("접근권한이 없습니다.");
		location.href='${pageContext.request.contextPath}/galleryboard/list.do';
	</c:if>
...
<body>
	...
	<form action="../galleryboard/edit.do" name="wirteFrm" method="post" enctype="multipart/form-data"	
			onsubmit="return validateForm(this)">
			<input type="hidden" name="id" id="id" value="${ id }" />
			<input type="hidden" name="nickname" id="nickname" value="${ nickname }" />
			<input type="hidden" name="cate" id="cate" value="gall"/>
			<!-- 게시물 수정을 위한 일련번호 -->
			<input type="hidden" name="gallno" value="${dto.gallno }" />
	...
	<table border="1" class="table" style="width: 80%; margin: auto;">
		...
		<tr>
			<td colspan="2" align="center">
				...
				<button class="btn-light sm" type="button" onclick="location.href='../galleryboard/list.do';">목록 바로가기</button>
			</td>
		</tr>
	</table>		
	</form>
