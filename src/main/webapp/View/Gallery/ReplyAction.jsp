	...
	<%
		...
		if(userID==null){
			...
			if(reply.getReplycontent()==null){
				...
				int result = replyDAO.write(gallno, userID, reply.getReplycontent()); 
				replyDAO.close();
				if(result==-1){
					PrintWriter script= response.getWriter();
					script.println("<script>");
					...
					String url="/galleryboard/lookup.do?gallno=";
					GalleryDAO dao = new GalleryDAO();
					dao.deleteVisitcount(request.getParameter("gallno"));
					dao.close();
					PrintWriter script= response.getWriter();
					script.println("<script>");
					script.println("location.href='"+request.getContextPath()+url+gallno+"'");
					script.println("</script>");
				}
			}
		}
