...
//탈퇴
function signout() {
	var result = confirm("회원의 모든글과 게시물이 삭제됩니다. 탈퇴처리하시겠습니까?")
	if (result) {
		location.href="${pageContext.request.contextPath}/admin/delete.do?id=${dto.id}";
	}
}
//비로그인 접근시 보안
$(function () {	
...
<body>
	...
	<br /><br />
	<form action="${pageContext.request.contextPath }/admin/infoedit.do" method="post" name="frm">
	<div id="wrap" style="margin: auto; width: 100%; height: 120%">
		<table class="table-fill">
			...
			<tbody class="table-hover">
				<tr>
					...
					<td class="text-left">${dto.id } <input type="hidden" name="id" id="id" value="${dto.id }" /></td>
				</tr>
				<tr>
		...
		<br>
            ...
            <button type="submit" class="btn btn-primary" onclick="return isValidate(frm);">변경하기</button>
            <button type="button" class="btn btn-danger" onclick="signout();">회원탈퇴</button>
            <button type="button" class="btn btn-info" href="${pageContext.request.contextPath }/home.do">홈으로</button>
            <br /><br /><br /><br />
            </div>
		</div>
	</form>
