...
public class FreeEditController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		dao.close();
		req.getRequestDispatcher("/View/freeboard/FreeboardEdit.jsp").forward(req, resp);
	}
...
protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String freeno = mr.getParameter("freeno");
		...
		if(result ==1) {
			...
			resp.sendRedirect("../freeboard/lookup.do?freeno="+freeno);
		}else {
			AlertFunction.alertLocation(resp, "비밀번호 검증을 다시 진행해주세요","../Freeboard/lookup.do?freeno="+freeno);
		}
	}
}
