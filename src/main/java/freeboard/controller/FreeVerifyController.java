...
@WebServlet("/freeboard/verify.do")
public class FreeVerifyController extends HttpServlet{
	...
	//패스워드 검증 페이지로 진입
		...
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			*/
			...
			req.setAttribute("freeno",req.getParameter("freeno"));
			req.setAttribute("mode", req.getParameter("mode"));
			req.getRequestDispatcher("/View/Freeboard/FreeboardVerify.jsp").forward(req, resp);
		}
		...
		protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			if (verify==0) {
				...
				if (mode.equals("edit")) {
					...
					resp.sendRedirect(req.getContextPath()+"/freeboard/edit.do?freeno="+freeno);
				}
				else if (mode.equals("delete")){
					System.out.println("delete모드 입장");
					FreeboardDAO noticedao = new FreeboardDAO();
					//기존 게시물의 내용을 읽어온다.
					FreeboardDTO noticedto = noticedao.lookup(freeno);
					...
					if((noticedto.getId().equals(session.getAttribute("id")) || session.getAttribute("id").equals("admin"))) {
						//게시물을 삭제한다.
						int result = noticedao.deletePost(freeno);
						dao.close();
						...
						//삭제후엔 리스트로 돌아온다.
						AlertFunction.alertLocation(resp, "삭제되었습니다", "../freeboard/list.do");
					
					}//게시판 id 와 session id가 다르면
					else if (!(noticedto.getId().equals(session.getAttribute("id")))) {
						AlertFunction.alertLocation(resp, "본인의 게시물만 삭제할 수 있습니다.","../freeboard/lookup.do?freeno="+freeno);
						return;
					}
